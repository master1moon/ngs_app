...
class NetworkCardsRepository(private val database: AppDatabase) {
    ...
    suspend fun getTotalExpensesByDateRange(fromDate: String, toDate: String): Double = 
        database.expenseDao().getTotalExpensesByDateRange(fromDate, toDate) ?: 0.0
    ...
    suspend fun getTotalSalesByDateRange(fromDate: String, toDate: String): Double = 
        database.saleDao().getTotalSalesByDateRange(fromDate, toDate) ?: 0.0
    ...
    suspend fun getTotalPaymentsByDateRange(fromDate: String, toDate: String): Double = 
        database.paymentDao().getTotalPaymentsByDateRange(fromDate, toDate) ?: 0.0
    ...
    /**
     * حساب رصيد المحل (المبيعات - التسديدات)
     */
    ...
    /**
     * حساب صافي الأرباح (إجمالي المبيعات - إجمالي المصروفات)
     */
    ...
    /**
     * حساب صافي الأرباح لفترة زمنية محددة
     */
